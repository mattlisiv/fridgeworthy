<footer>
    <div class="footercnt">
        <ul>
            <li class="footertxt">Copyright © 2014 FridgeWorthy, LLC</li>
            <li class="footertxt"><a href="/privacypolicy">Privacy Policy</a></li>
            <li class="footertxt"><a href="{{action('HomeController@TermsAndConditions')}}">Terms and Conditions</a></li>
            <li class="footertxt"><span>phone:</span>  <a href="tel:404-444-8551">404-444-8551</a></li>
            <li class="footertxt"><span>email:</span>  <a href="mailto:support@fridge-worthy.com">support@fridge-worthy.com</a></li>
            <li class="footertxt" style="width: 25%"><span></span></li>
            <li class="footertxt" style="padding:3px"><a href="https://www.facebook.com/fridgeworthyrewards"><img src="{{asset('images/fblogo50.png')}}"></a></li>
            <li class="footertxt" style="padding:3px"><a href="https://twitter.com/fridge_worthy"><img width="50em" height="50em" src="{{asset('images/twitterlogo.png')}}"></a></li>
            <li class="footertxt" style="padding:3px"><a href="https://www.linkedin.com/company/fridgeworthy"><img width="50em" height="50em" src="{{asset('images/linkedin.png')}}"></a></li>
        </ul>
    </div>
</footer>
<!--end footer-->
